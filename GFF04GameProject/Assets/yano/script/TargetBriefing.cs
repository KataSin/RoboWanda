...
public class TargetBriefing : MonoBehaviour
{
    [SerializeField]
    ...
    void Update()
    {
        ...
        target_.transform.localPosition =
            Vector3.Lerp(new Vector3(-211f, 66f, 0f), new Vector3(0f, 66f, 0f), t2 / 1f);

        if (!isClear)
        {
            if (!isTarget)
                target_.transform.localScale =
                    Vector3.Lerp(Vector3.zero, new Vector3(0.2f, 4f, 0f), t2 / 1f);
            else
                target_.transform.localScale =
                    Vector3.Lerp(new Vector3(0.2f, 4f, 0f), new Vector3(2f, 4f, 0f), t2 - 1f / 1f);
        }
        else
        {
            if (!isTarget)
